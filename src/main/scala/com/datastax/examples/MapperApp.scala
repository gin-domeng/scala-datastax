...
import com.datastax.examples.killrvideo.user.{User, UserDao}
import com.datastax.examples.killrvideo.video.{LatestVideo, UserVideo, Video, VideoByTag, VideoDao}
...
object MapperApp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // Source.fromResource(CqlSession.builder().build() ) { session =>
    CqlSession.builder()
      .addContactPoint(new InetSocketAddress("127.0.0.1", 9042))
      .withLocalDatacenter("datacenter1") { session =>
  ...
  private def getStatements(fileName: String): List[String] = {
   ...
   /* Files.readString(path).split(";")
    ...
    Source.fromFile(path.toFile).getLines.toString().split(";").toList.map(_.trim)
  }
